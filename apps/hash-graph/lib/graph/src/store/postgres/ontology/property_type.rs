...
use crate::{
    ontology::{OntologyElementMetadata, PropertyTypeQueryPath, PropertyTypeWithMetadata},
...
impl<C: AsClient> PostgresStore<C> {
    pub(crate) async fn read_property_types_by_ids(
        &self,
        vertex_ids: impl IntoIterator<Item = PropertyTypeVertexId> + Send,
        temporal_axes: &QueryTemporalAxes,
    ...
    pub(crate) async fn traverse_property_types(
        &self,
        mut property_type_queue: Vec<(PropertyTypeVertexId, GraphResolveDepths)>,
        ...
        while !property_type_queue.is_empty() {
            ...
            for (property_type_vertex_id, graph_resolve_depths) in property_type_queue.drain(..) {
                for edge_kind in [
                        ...
                        .decrement_depth_for_edge(edge_kind, EdgeDirection::Outgoing)
                    {
                        ...
                        entry.0.push(property_type_vertex_id.base_id.to_string());
                        entry.1.push(property_type_vertex_id.revision_id);
                ...
                edges_to_traverse.get(&OntologyEdgeKind::ConstrainsValuesOn)
            {
                data_type_queue.extend(
                    ...
                    .map(|(source, target, index)| {
                        let source_vertex_id = PropertyTypeVertexId::from(source);
                        let target_vertex_id = DataTypeVertexId::from(target);
